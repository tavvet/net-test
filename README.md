...
Те же метрики доступны и как **Android-приложение** (GUI на
[Fyne](https://fyne.io), то же ядро) — см. [Android](#android).
...
Вкладка «Маршрут» — сверху строка-вердикт «**где начинаются потери**», а у
каждого хопа цветной бар потерь; проблемный узел помечен `⚠` (потери держатся до
конца), рядом с IP видно AS-имя сети, а в «Сред» — прирост задержки `+ΔX`
...
⚠ Потери начинаются на хопе 4 · CLOUDFLARENET
     ...
     3  212.237.216.242 · ITNET-AS                ░░░░░░ 0%    42   13.0     13.1   12.0   28.0    3.2
...
То же ядро под Android — GUI на [Fyne](https://fyne.io), четыре вкладки тем же
`internal/probe`. ICMP работает тем же unprivileged-сокетом (нужен лишь `INTERNET`
permission, Fyne добавляет его сам) — проверено на реальном устройстве. Вкладка
«Маршрут» — визуальная карта потерь: баннер «где начинаются потери» и цветной бар
у каждого хопа (та же логика, что в TUI, общий `probe.Quality`).

<p>
  <img src="docs/img/tab1-ping.png"  width="200" alt="Пинг">
  <img src="docs/img/tab2-trace.png" width="200" alt="Маршрут">
  <img src="docs/img/tab3-diag.png"  width="200" alt="Диагноз">
  <img src="docs/img/tab4-speed.png" width="200" alt="Скорость">
</p>

Сборка APK — **без Android Studio**, нужны только Android SDK + NDK (пути ищутся
автоматически под `~/Library/Android/sdk` или задаются через
`ANDROID_HOME`/`ANDROID_NDK_HOME`):

```bash
make apk          # → dist/net-test.apk (arm64; все ABI: ANDROID_ABIS=android)
make gui          # то же приложение десктоп-окном (отладка UI)
make test-mobile  # тесты Fyne-приложения
```

Установить на телефон: `adb install dist/net-test.apk` (или скинуть APK и
разрешить установку из неизвестных источников). Как устроен мобильный слой —
...
| Android (arm64) | ✅ GUI на [Fyne](https://fyne.io), `make apk` без Android Studio; проверено на устройстве |
...
make apk                    # Android APK (Fyne) → dist/net-test.apk (см. «Android»)
make help                   # все цели
```
...
make test-mobile   # тесты Android/Fyne-приложения (CGO)
