...
потери, джиттер, где именно на маршруте начинаются обрывы, и реальная скорость
(download/upload)** — всё в одном окне терминала.
...
|---|---|
| **1 · Пинг** | Живой монитор до цели: текущий/средний RTT, потери %, джиттер (RFC 3550), история проб в виде спарклайна (`✕` = потеря) и вердикт качества. |
| **2 · Маршрут** | mtr-стиль: пингует каждый хоп по пути и показывает per-hop потери, last/avg/best/worst и СКО — видно, **на каком участке** начинаются проблемы. Хосты резолвятся в фоне. |
| **3 · Скорость** | Реальный замер через открытые endpoint'ы Cloudflare: латентность, download ↓ и upload ↑ в Mbps, плюс датацентр CF и ваш IP. |
...
| Клавиша | Действие |
|---|---|
| `1` `2` `3` / `Tab` / `←` `→` | переключение вкладок |
| `s` | запустить (перезапустить) тест скорости |
| `q` / `Ctrl-C` | выход |

## Как выглядит

Вкладка «Маршрут» — проблемный хоп №4 (потери 18%, скачок задержки) сразу виден
(в реальном терминале — с цветовой подсветкой):

```
 net-test    цель 1.1.1.1  │  CF MXP IT  │  время 0:42
  1 Пинг    2 Маршрут    3 Скорость

  #  Хост / IP                  Потери  Отпр  Послед   Сред   Лучш   Худш    СКО
  1  router.lan (10.0.1.1)        0%     42    1.2     1.4    0.9    3.1    0.4
  2  5.180.172.2                  0%     42   12.0    11.8   10.1   30.2    5.1
  3  212.237.216.242              0%     42   14.1    13.9   11.0   28.0    4.7
  4  162.158.236.14              18%     42   85.1    42.1   14.0  190.3   38.2
  5  162.158.236.11               2%     42   19.8    24.0   16.3   61.0    9.3
  6  one.one.one.one (1.1.1.1)    0%     42   14.5    15.2   11.8   33.0    6.0

  1-3/⭾ вкладки   s тест скорости   q выход
```
