...
- `Diagnosis.Healthy` и `FirstIssue` считаются один раз — UI и JSON не обходят
  сегменты заново.
- `Diagnosis.FirstIssueHop` (TTL виновного хопа) и `FirstIssueLoss` (потери это
  или скачок задержки) дают вкладке «Маршрут» структурный ответ «где начинаются
  потери» — баннер и цветной бар у каждого хопа рисуются по ним и по
  `probe.Quality`, одинаково в TUI и в мобильном GUI.
